...
-- Credit: filiptibell (https://devforum.roblox.com/t/coding-challenge-8-snakecase-to-camelcase/668949/2)
function camel() manipulate(".*",
    function (s)
        return string.gsub(s, " %w+", function(word)
...
function snake() manipulate(".*",
    function (s)
        return string.gsub(s, " %w+", function(word)
            return "_" .. string.lower(string.sub(word, 2))
        end)
    end
) end
...
function init()
    ...
    config.MakeCommand("title", capital, config.NoComplete)
